...
// ─────────────────────────────────────────────────────────────────────────────
// RASOAF TRAVELS AND TOURS LIMITED — Premium Hero (v30)
// Optimized: Font preloading · Reduced GC pressure · 60fps · Memory cleanup
// ─────────────────────────────────────────────────────────────────────────────
...
const MARQUEE_PANELS = Object.freeze([...PANELS, ...PANELS]);

const BG_VIDEOS = Object.freeze([
...
// ══════════════════════════════════════════════════════════════════════════
// Premium CSS — Fonts loaded via <link> in HTML head
// ══════════════════════════════════════════════════════════════════════════
const CSS = `
  .rh-root {
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .rh-headline .rh-char { display: inline-block; }
  .rh-headline em {
  ...
  .rh-cta-primary:active { transform: translateY(0) scale(0.98) translateZ(0); box-shadow: 0 2px 12px rgba(212,160,23,0.25); }
  ...
  .rh-cta-secondary:active { transform: translateY(0) scale(0.98) translateZ(0); }
  ...
  .rh-trust-badge:hover {
    ...
    color: var(--clr-accent); transform: translateY(-2px) translateZ(0);
  ...
  .rh-panel:hover {
    ...
    transform: rotate(var(--panel-rotate, 0deg)) translateY(-6px) translateZ(0);
  }
  .rh-panel img {
  ...
  .rh-panel:hover img { transform: scale(1.18) translateZ(0); }
  ...
  @keyframes rh-marquee-css { from { transform: translateX(0) translateZ(0); } to { transform: translateX(-50%) translateZ(0); } }
  ...
  .rh-tv-container {
    ...
    width: 100%; max-width: 420px; margin: 0 auto; z-index: 200;
  }
  .rh-tv-screen {
  ...
  .rh-tv-control-btn {
    ...
    transition: transform 0.3s ease, background 0.3s ease, border-color 0.3s ease; padding: 0;
  }
  ...
  @media (max-width: 768px) {
    ...
    .rh-tv-container { max-width: 300px; }
...
// ══════════════════════════════════════════════════════════════════════════
// Trust Badge — Memoized
// ══════════════════════════════════════════════════════════════════════════
const TrustBadge = memo(function TrustBadge({ text }) {
  return (
    <div className="rh-trust-badge" role="listitem">
...
// ══════════════════════════════════════════════════════════════════════════
// Marquee Panel — Memoized
// ══════════════════════════════════════════════════════════════════════════
const MarqueePanel = memo(function MarqueePanel({ panel, index, total, onSelect }) {
...
export default function RasoafHero() {
  ...
  useEffect(() => {
    ...
    mm.add("(prefers-reduced-motion: no-preference)", () => {
      ...
      const pe = promoVideoRef.current;
      const sa = bgSlideARef.current;
      const sb = bgSlideBRef.current;
      if (!ve || !sa || !sb || !pe) return;

      pe.muted = true;
      pe.playsInline = true;
      pe.style.opacity = "0";
      pe.className = "rh-bgslide";
      ve.src = BG_VIDEOS[0];
      ...
      if (isMobileRef.current) {
        ...
        const fs = BG_SLIDES[0];
        sa.src = fs.src;
        sa.style.objectPosition = fs.position;
        gsap.set(sa, { opacity: 1, className: "rh-bgslide active" });

        let mct = null;
        const rmc = () => {
          const idx = mobileSlideIdxRef.current % BG_SLIDES.length;
          const sd = BG_SLIDES[idx];
          const se = bgSlideIndex.current % 2 === 0 ? sb : sa;
          const pse = bgSlideIndex.current % 2 === 0 ? sa : sb;

          se.src = sd.src;
          se.style.objectPosition = sd.position;
          gsap.set(se, { opacity: 0, scale: 1.04, className: "rh-bgslide active" });
          gsap.to(se, { opacity: 1, scale: 1, duration: 1.2, ease: "power2.out" });

          if (pse.classList.contains("active")) {
            gsap.to(pse, {
              opacity: 0,
              scale: 1.02,
              duration: 1.5,
              ease: "power2.inOut",
              onComplete: () => gsap.set(pse, { className: "rh-bgslide" }),
            });
          }
      ...
      // Desktop: video → promo → image rotation
      let nsi = 0;
      let ct = null;

      const ppv = () => {
        const vd = VIDEO_SLIDES[videoSlideIndex.current % VIDEO_SLIDES.length];
        pe.src = vd.src;
        pe.load();
      ...
      const rc = async () => {
        if (bgModeRef.current === "video") {
          ...
          gsap.to(ve, {
            ...
            duration: 1.5,
            ease: "power3.inOut",
            onComplete: () => gsap.set(ve, { className: "rh-bgvid" }),
          });
          gsap.to(pe, { opacity: 1, scale: 1, duration: 2.0, ease: "power3.out" });
          await ppv();
          ct = gsap.delayedCall(0.5, rc);
        } else if (bgModeRef.current === "promo") {
          const sd = BG_SLIDES[nsi % BG_SLIDES.length];
          const se = bgSlideIndex.current % 2 === 0 ? sa : sb;
          se.src = sd.src;
          se.style.objectPosition = sd.position;
          gsap.set(se, { opacity: 0, scale: 1.04, className: "rh-bgslide active" });
          gsap.to(pe, {
            ...
            duration: 1.8,
            ease: "power3.inOut",
            onComplete: () => gsap.set(pe, { className: "rh-bgslide" }),
          });
          ...
          const nbv = BG_VIDEOS[bgVideoIndex.current % BG_VIDEOS.length];
          ve.src = nbv;
  ...
  useEffect(() => {
    ...
    mm.add("(prefers-reduced-motion: no-preference)", () => {
      const ctx = gsap.context(() => {
        ...
        tl.to(ctaRowRef.current, { opacity: 1, y: 0, duration: 0.8 }, 0.8);
        tl.to(trustRef.current, { opacity: 1, y: 0, duration: 0.8 }, 0.95);

        const cb = ctaRowRef.current?.querySelectorAll(
        ...
        const bd = trustRef.current?.querySelectorAll(".rh-trust-badge");
        if (bd?.length) {
          tl.fromTo(
            bd,
          ...
          const rs = () =>
            gsap.fromTo(
              ...
              { left: "-60%" },
              {
                ...
                onComplete: () => {
                  gsap.set(se, { left: "-60%" });
                  gsap.delayedCall(10, rs);
                },
              }
            );
          gsap.delayedCall(2, rs);
        }
      }, rootRef);
  ...
  const handleRegisterClick = useCallback(() => scrollTo("register"), [scrollTo]);
      ...
      <div className="rh-root" ref={rootRef}>
        ...
        >
          ...
          <div className="rh-content">
            ...
            >
              {TRUST_BADGES.map((b) => (
                <TrustBadge key={b.text} text={b.text} />
              ))}
            </div>
          </div>
          ...
          <div className="rh-tv-section" ref={tvSectionRef}>
            <div className="rh-tv-container" ref={tvContainerRef}>
              <div className="rh-tv-screen">
                {tvVideoError ? (
                  <div className="rh-tv-error" role="status">
                    <span className="rh-tv-error__icon" aria-hidden="true">
                      ✈️
                    </span>
                    <p className="rh-tv-error__title">Video loading…</p>
                    <p className="rh-tv-error__sub">
                      Premium content will appear shortly
                    </p>
                  </div>
                ) : (
                  <video
                ...
                {!tvVideoError && (
                  <div className="rh-tv-controls">
                    ...
                    <button
                      ...
                      aria-label={isTvMuted ? "Unmute" : "Mute"}
                    >
                      {isTvMuted ? (
                        <VolumeX size={16} />
                      ) : (
                ...
                <div className="rh-tv-badge" aria-hidden="true">
                  ✈️ WANDERLUST
                </div>
              </div>
