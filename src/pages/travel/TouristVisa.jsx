...
const travelPurposes = ["Holiday / Vacation", "Sightseeing & Tours", "Cultural Exploration", "Family Visit", "Honeymoon", "Other"];

const tourFeaturesData = [
  { icon: MapPin, title: "Sightseeing Tours" }, { icon: Camera, title: "Cultural Attractions" },
  { icon: Sun, title: "Holiday & Vacation" }, { icon: Plane, title: "Amusement Parks" },
];
    ...
    <div style={s.page}>
      ...
      <section style={s.contentSection}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          ...
          <div style={s.featureGrid}>{tourFeaturesData.map((feat, idx) => { const Icon = feat.icon; return (<motion.div key={idx} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.08 }} viewport={{ once: true }} style={s.featureCard} whileHover={!isMobile ? { borderColor: brand.gold, boxShadow: "0 8px 24px rgba(0,0,0,0.06)" } : {}}><div style={s.featureIcon}><Icon size={22} color={brand.goldDark} /></div><h4 style={s.featureTitle}>{feat.title}</h4></motion.div>); })}</div>
        </motion.div>
