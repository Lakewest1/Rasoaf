...
const studyLevels = ["Bachelor's Degree", "Master's Degree", "PhD / Doctorate", "Diploma / Certificate", "Exchange Programme", "Other"];
const intakeOptions = ["January / Spring", "May / Summer", "September / Fall", "Flexible / Any"];
...
const s = {
  ...
  // Country Grid
  countryGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(clamp(240px, 30vw, 280px), 1fr))", gap: "clamp(14px, 2vw, 20px)", marginTop: "clamp(16px, 2.5vw, 24px)" },
  countryCard: { background: brand.cream, borderRadius: "clamp(14px, 1.5vw, 16px)", padding: "clamp(18px, 2.5vw, 24px)", border: `1px solid ${brand.borderLight}`, transition: "all 0.3s ease" },
  countryName: { fontSize: "clamp(16px, 1.5vw, 18px)", fontWeight: 700, color: brand.dark, fontFamily: "'Manrope', sans-serif", marginBottom: "clamp(8px, 1vw, 12px)", display: "flex", alignItems: "center", gap: "clamp(6px, 0.8vw, 8px)" },
...
function StudentVisaForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const sd = { _subject: `New Student Visa - ${formData.firstName} ${formData.lastName}`, "First Name": formData.firstName, "Last Name": formData.lastName, "Email": formData.email, "Phone": fp, "Destination": formData.destinationCountry, "Study Level": formData.studyLevel, "Intake": formData.preferredIntake || "N/S", "Message": formData.message || "N/A", "Submitted At": new Date().toLocaleString(), "Page": window.location.href };
      ...
      setTimeout(() => { setSubmitted(false); setFormData({ firstName: "", lastName: "", email: "", phoneCode: "+234", phone: "", destinationCountry: "", studyLevel: "", preferredIntake: "", message: "" }); }, 6000);
  ...
  return (
    <div style={s.page}>
      ...
      <section style={s.heroSection}>
        <div style={s.heroBg}><img src="https://images.unsplash.com/photo-1523050854058-8df90910e683?w=1920&h=800&fit=crop" alt="Students" style={s.heroImg} loading="eager" /></div>
        <div style={s.heroOverlay} />
        <div style={s.heroContent}><motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}><span style={s.badge}><GraduationCap size={14} color={brand.gold} /><span style={s.badgeText}>STUDY ABROAD</span></span><h1 style={s.heroTitle}>{STUDENT_VISA.heading}</h1><p style={s.heroSubtitle}>{STUDENT_VISA.intro}</p><div style={s.heroBtnRow}><button onClick={scrollToForm} style={s.heroBtn} onMouseEnter={e => { e.currentTarget.style.backgroundColor = "#FFE082"; e.currentTarget.style.boxShadow = "0 6px 28px rgba(247,201,72,0.5)"; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = brand.goldLight; e.currentTarget.style.boxShadow = "0 4px 20px rgba(247,201,72,0.35)"; }}><GraduationCap size={20} />Apply Now<ArrowDown size={18} /></button></div></motion.div></div>
      </section>
      ...
      <section style={s.contentSection}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.ctaBox}>
          ...
          <div style={s.ctaContent}><h3 style={s.ctaTitle}>Ready to Start?</h3><p style={s.ctaText}>Contact us today. Call {CONTACT_INFO.phone} or email {CONTACT_INFO.email}</p><button onClick={scrollToForm} style={s.ctaButton} onMouseEnter={!isMobile ? e => e.target.style.backgroundColor = "#FFE082" : undefined} onMouseLeave={!isMobile ? e => e.target.style.backgroundColor = brand.white : undefined}>Apply Now <ArrowRight size={16} /></button></div>
        </motion.div>

        {/* FORM */}
        <div id="visa-booking-form" style={{ marginTop: "clamp(24px, 4vw, 40px)" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }}><StudentVisaForm /></motion.div>
        </div>
      </section>
