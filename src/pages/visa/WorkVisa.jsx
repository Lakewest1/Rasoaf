...
const jobTypes = ["Skilled Worker / Professional", "Healthcare / Medical", "Engineering", "IT / Technology", "Education / Teaching", "Hospitality / Tourism", "Construction / Trades", "Other"];
const destinationCountries = ["United Kingdom", "Canada", "United States", "Australia", "United Arab Emirates", "Saudi Arabia", "Ireland", "Germany", "Other"];
...
const s = {
  ...
  // Documents
  docGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(clamp(240px, 30vw, 280px), 1fr))", gap: "clamp(8px, 1vw, 12px)", marginTop: "clamp(12px, 2vw, 20px)" },
  docItem: { display: "flex", alignItems: "flex-start", gap: "clamp(6px, 1vw, 10px)", padding: "clamp(12px, 1.5vw, 14px) clamp(12px, 1.5vw, 16px)", background: brand.cream, borderRadius: "clamp(10px, 1vw, 12px)", fontSize: "clamp(12px, 1vw, 14px)", color: brand.mutedText, lineHeight: 1.5 },
...
function WorkVisaForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => { setSubmitted(false); setFormData({ firstName: "", lastName: "", email: "", phoneCode: "+234", phone: "", destinationCountry: "", jobType: "", message: "" }); }, 6000);
    } catch (err) { setLoading(false); setFormError(err.message || "Something went wrong."); setTimeout(() => setFormError(null), 8000); }
  };

  const inp = (n) => ({ ...s.input, ...(focused === n ? s.focusInput : {}), ...(formError && !formData[n] && ["firstName","lastName","email","phone"].includes(n) ? s.errorInput : {}) });
  ...
  return (
    <div style={s.page}>
      ...
      <section style={s.heroSection}>
        <div style={s.heroBg}><img src="https://images.unsplash.com/photo-1551836022-d5d88e9218df?w=1920&h=800&fit=crop" alt="Office" style={s.heroImg} loading="eager" /></div>
        <div style={s.heroOverlay} />
        <div style={s.heroContent}><motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}><span style={s.badge}><Briefcase size={14} color={brand.gold} /><span style={s.badgeText}>EMPLOYMENT ABROAD</span></span><h1 style={s.heroTitle}>{WORK_VISA.heading}</h1><p style={s.heroSubtitle}>{WORK_VISA.intro}</p><div style={s.heroBtnRow}><button onClick={scrollToForm} style={s.heroBtn} onMouseEnter={e => { e.currentTarget.style.backgroundColor = "#FFE082"; e.currentTarget.style.boxShadow = "0 6px 28px rgba(247,201,72,0.5)"; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = brand.goldLight; e.currentTarget.style.boxShadow = "0 4px 20px rgba(247,201,72,0.35)"; }}><Briefcase size={20} />Get Started<ArrowDown size={18} /></button></div></motion.div></div>
      </section>
      ...
      <section style={s.contentSection}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          ...
          <CollapsibleText text={WORK_VISA.description} isMobile={isMobile} />
        </motion.div>

        {/* Documents */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          <h2 style={s.sectionTitle}><FileText size={22} style={{ display: "inline", marginRight: "8px", verticalAlign: "middle" }} />Required Documents</h2>
          <CollapsibleText text="Depending on the destination country and employer requirements, applicants may be required to provide the following documents:" isMobile={isMobile} />
          <div style={s.docGrid}>{WORK_VISA.requiredDocuments.map((doc, idx) => (<motion.div key={idx} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} viewport={{ once: true }} style={s.docItem}><CheckCircle size={16} style={s.docIcon} /><span>{doc}</span></motion.div>))}</div>
        </motion.div>

        {/* Closing */}
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          <CollapsibleText text={WORK_VISA.closing} isMobile={isMobile} />
        </motion.div>
