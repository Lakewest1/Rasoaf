...
const s = {
  ...
  purposeList: { display: "flex", flexDirection: "column", gap: "10px", marginTop: "16px" },
  purposeItem: { display: "flex", alignItems: "center", gap: "10px", padding: "12px 16px", background: brand.cream, borderRadius: "10px", fontSize: "14px", color: brand.mutedText, border: `1px solid ${brand.borderLight}` },
...
@media (max-width: 768px) {
  ...
  .purpose-list { gap: 8px !important; }
}
...
function BusinessVisaForm() {
  const [formData, setFormData] = useState({
    ...
    destination: "", purpose: "", company: "", message: "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionData = {
        _subject: `New Business Visa Enquiry - ${formData.firstName} ${formData.lastName}`,
  ...
  const inp = (name) => ({ ...s.input, ...(focused === name ? s.focusInput : {}), ...(formError && !formData[name] && ["firstName","lastName","email","phone","destination","purpose"].includes(name) ? s.errorInput : {}) });
    ...
    return (
      <div style={s.successWrapper}>
        <div style={s.successImgSide}>
          <img src="https://images.unsplash.com/photo-1556761175-5973dc0f32e7?w=600&h=900&fit=crop" alt="" style={{ ...s.formImageBg, opacity: 0.35 }} />
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} style={s.successContent}>
          ...
          <h3 style={s.successTitle}>Business Enquiry Submitted!</h3>
          <p style={s.successMsg}>Your business visa enquiry has been received. Our corporate travel specialist will contact you within <strong>24 hours</strong>.</p>
  ...
  return (
    <div style={s.formWrapper} className="form-flex-container">
      <div style={s.formImageSide} className="form-image-side">
        <img src="https://images.unsplash.com/photo-1556761175-5973dc0f32e7?w=600&h=900&fit=crop" alt="Business meeting" style={s.formImageBg} />
        <div style={s.formImageOverlay} />
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={s.formImageContent}>
          <div style={s.formImageBadge}><Sparkles size={12} color={brand.gold} /><span>CORPORATE TRAVEL</span></div>
          <div style={s.formImageIcon}><Briefcase size={34} color={brand.gold} /></div>
          <h2 style={s.formImageTitle}>Business <span style={s.formImageGold}>Visa</span><br />Application</h2>
          <p style={s.formImageSubtitle}>Conferences, meetings, trade shows – we handle your business visa so you can focus on your goals.</p>
          <div style={s.formDivider} />
          <div style={s.formImageStats}>
            <div style={s.formImageStat}><div style={s.formImageStatVal}>300+</div><div style={s.formImageStatLbl}>Corporate Clients</div></div>
            <div style={s.formImageStat}><div style={s.formImageStatVal}>10+</div><div style={s.formImageStatLbl}>Countries</div></div>
            <div style={s.formImageStat}><div style={s.formImageStatVal}>98%</div><div style={s.formImageStatLbl}>Success Rate</div></div>
          </div>
          <div style={s.formImageFeatures}>
            {[
              { icon: Shield, text: "Licensed Immigration Advisers" },
              { icon: Users, text: "Group/Delegation Support" },
              { icon: Globe, text: "Multi‑country Itineraries" },
            ].map((item, i) => {
              const I = item.icon;
      ...
      <div style={s.formSide} className="form-form-side">
        ...
        >
          <div style={s.formHeaderStyle}>
            ...
            <div><div style={s.formHeaderTitle}>Start Your Business Visa</div><div style={s.formHeaderSub}>Fill the form below to get started</div></div>
          </div>
          ...
          <div style={s.formRow}>
            <div style={s.formGroup}>
              <label style={s.label}>Purpose of Travel <span style={s.required}>*</span></label>
              <select name="purpose" value={formData.purpose} onChange={handleChange} required style={s.select}>
                <option value="">Select...</option>
                {businessPurposes.map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div style={s.formGroup}>
              <label style={s.label}>Company / Organization</label>
              <input type="text" name="company" value={formData.company} onChange={handleChange} placeholder="Your company name" style={inp("company")} onFocus={() => setFocused("company")} onBlur={() => setFocused(null)} />
            </div>
          </div>

          <div style={s.formRow}><div style={s.formGroup}><label style={s.label}>Additional Message</label><textarea name="message" value={formData.message} onChange={handleChange} rows={3} placeholder="Any special requirements or dates..." style={s.textarea} onFocus={() => setFocused("message")} onBlur={() => setFocused(null)} /></div></div>
    ...
    <div style={s.page}>
      ...
      <section style={s.heroSection} className="hero-section">
        ...
        <div style={s.heroContent}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <span style={s.badge}>
              <Globe size={14} color={brand.gold} />
              <span style={s.badgeText}>BUSINESS TRAVEL</span>
            </span>
            <h1 style={s.heroTitle} className="hero-title">{BUSINESS_VISA.heading}</h1>
            <p style={s.heroSubtitle}>{BUSINESS_VISA.intro}</p>
      ...
      <section style={s.contentSection}>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          ...
          <p style={s.text}>We assist clients in preparing and organizing the necessary documentation required for various business-related travel purposes, including:</p>
          <div style={s.purposeList} className="purpose-list">
            {BUSINESS_VISA.purposes.map((purpose, idx) => {
              const icons = [Presentation, Users, Globe, Briefcase, Users];
              const Icon = icons[idx % icons.length];
              return (
                <motion.div key={idx} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.06 }} viewport={{ once: true }} style={s.purposeItem}>
                  <Icon size={16} style={s.purposeIcon} /><span>{purpose}</span>
                </motion.div>
              );
            })}
          </div>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          ...
          <p style={{ ...s.text, marginTop: "20px" }}>{BUSINESS_VISA.usInfo}</p>
        </motion.div>

        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          <p style={s.text}>{BUSINESS_VISA.closing}</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.ctaBox}>
          ...
          <div style={s.ctaContent}>
            <h3 style={s.ctaTitle}>Plan Your Business Trip</h3>
            <p style={s.ctaText}>Contact us today for business visa assistance. Call {CONTACT_INFO.phone} or email {CONTACT_INFO.email}</p>
