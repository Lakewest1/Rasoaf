...
const STUDENT_VISA = {
  ...
  description:
    "RASOAF Travels and Tours Limited provides comprehensive student visa support for Nigerian students aspiring to study in top destinations around the world. From document preparation to visa interview coaching, our experienced consultants are with you every step of the way.",
  financialGuidance:
    "We also assist with proof of funds documentation, scholarship applications, and guidance on maintaining the required bank balance for your chosen destination.",
  closing:
    "Whether you are pursuing a bachelor’s, master’s, or doctoral degree, our team ensures that your visa application is accurate, complete, and submitted on time. Let us handle the paperwork while you prepare for your academic journey.",
  countries: [
    {
      name: "United Kingdom",
      services: [
        "Tier 4 (General) Student Visa",
        "CAS & University Admission Support",
        "NHS Surcharge Assistance",
        "Dependants Visa Guidance"
      ]
    },
    {
      name: "Canada",
      services: [
        "Study Permit & SDS Stream",
        "Designated Learning Institution (DLI) Verification",
        "Proof of Funds Documentation",
        "Post‑Graduation Work Permit Info"
      ]
    },
    {
      name: "United States",
...
const s = {
  ...
  countryGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "20px", marginTop: "24px" },
  countryCard: { background: brand.cream, borderRadius: "16px", padding: "24px", border: `1px solid ${brand.borderLight}`, transition: "all 0.3s ease" },
  countryName: { fontSize: "18px", fontWeight: 700, color: brand.dark, fontFamily: "'Manrope', sans-serif", marginBottom: "12px", display: "flex", alignItems: "center", gap: "8px" },
  featureList: { display: "flex", flexDirection: "column", gap: "8px" },
  featureItem: { display: "flex", alignItems: "flex-start", gap: "8px", fontSize: "13px", color: brand.mutedText, lineHeight: 1.5 },
  ...
  // Form styles (reused from other pages, adjusted)
...
// ══════════════════════════════════════════════════════════════════════════
//  STUDENT VISA BOOKING FORM COMPONENT
// ══════════════════════════════════════════════════════════════════════════
function StudentVisaForm() {
  const [formData, setFormData] = useState({
    firstName: "", lastName: "", email: "", phoneCode: "+234", phone: "",
    destinationCountry: "", studyLevel: "", preferredIntake: "",
    message: ""
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionData = {
        _subject: `New Student Visa Enquiry - ${formData.firstName} ${formData.lastName}`,
      ...
      setTimeout(() => {
        ...
        setFormData({
          firstName: "", lastName: "", email: "", phoneCode: "+234", phone: "",
          destinationCountry: "", studyLevel: "", preferredIntake: "", message: ""
        });
      }, 6000);
    } catch (error) {
      setLoading(false);
    ...
    return (
      <div style={s.successWrapper}>
        <div style={s.successImgSide}>
          <img src="https://images.unsplash.com/photo-1523050854058-8df90910e683?w=600&h=900&fit=crop" alt="" style={{ ...s.formImageBg, opacity: 0.35 }} />
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} style={s.successContent}>
          ...
          <p style={s.successMsg}>Your student visa enquiry has been received. Our education consultant will contact you within <strong>24 hours</strong> to discuss your study abroad plans.</p>
  ...
  return (
    <div style={s.formWrapper} className="form-flex-container">
      ...
      <div style={s.formImageSide} className="form-image-side">
        <img src="https://images.unsplash.com/photo-1523050854058-8df90910e683?w=600&h=900&fit=crop" alt="Students on campus" style={s.formImageBg} />
        <div style={s.formImageOverlay} />
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={s.formImageContent}>
          <div style={s.formImageBadge}><Sparkles size={12} color={brand.gold} /><span>YOUR FUTURE STARTS HERE</span></div>
          <div style={s.formImageIcon}><GraduationCap size={34} color={brand.gold} /></div>
          <h2 style={s.formImageTitle}>Student <span style={s.formImageGold}>Visa</span><br />Application</h2>
          <p style={s.formImageSubtitle}>Expert guidance for Nigeria, Ghana, UK, Canada, USA, and Australia. Let us help you achieve your academic dreams.</p>
          <div style={s.formDivider} />
          <div style={s.formImageStats}>
            <div style={s.formImageStat}><div style={s.formImageStatVal}>500+</div><div style={s.formImageStatLbl}>Students Placed</div></div>
            <div style={s.formImageStat}><div style={s.formImageStatVal}>10+</div><div style={s.formImageStatLbl}>Partner Countries</div></div>
            <div style={s.formImageStat}><div style={s.formImageStatVal}>98%</div><div style={s.formImageStatLbl}>Visa Success</div></div>
          </div>
          <div style={s.formImageFeatures}>
            {[
              { icon: Shield, text: "Licensed Immigration Consultants" },
              { icon: Globe, text: "Global University Partnerships" },
              { icon: Clock, text: "End-to-End Application Support" }
            ].map((item, i) => {
              const I = item.icon;
      ...
      <div style={s.formSide} className="form-form-side">
        ...
        >
          <div style={s.formHeaderStyle}>
            ...
            <div><div style={s.formHeaderTitle}>Start Your Application</div><div style={s.formHeaderSub}>Fill the form below to get started</div></div>
          </div>
          ...
          <div style={s.formRow}>
            <div style={s.formGroup}>
              <label style={s.label}>Study Level <span style={s.required}>*</span></label>
              <select name="studyLevel" value={formData.studyLevel} onChange={handleChange} required style={s.select}>
                <option value="">Select...</option>
            ...
            <div style={s.formGroup}>
              ...
              <select name="preferredIntake" value={formData.preferredIntake} onChange={handleChange} style={s.select}>
                <option value="">Select...</option>
          ...
          <div style={s.formRow}><div style={s.formGroup}><label style={s.label}>Additional Message</label><textarea name="message" value={formData.message} onChange={handleChange} rows={3} placeholder="Any specific questions or university preferences..." style={s.textarea} onFocus={() => setFocused("message")} onBlur={() => setFocused(null)} /></div></div>
  ...
  return (
    <div style={s.page}>
      ...
      <section style={s.heroSection} className="hero-section">
        <div style={s.heroBg}>
          <img
            src="https://images.unsplash.com/photo-1523050854058-8df90910e683?w=1920&h=800&fit=crop"
        ...
        <div style={s.heroContent}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <span style={s.badge}>
              <GraduationCap size={14} color={brand.gold} />
              <span style={s.badgeText}>STUDY ABROAD</span>
            </span>
            <h1 style={s.heroTitle} className="hero-title">{STUDENT_VISA.heading}</h1>
            <p style={s.heroSubtitle}>{STUDENT_VISA.intro}</p>
      ...
      <section style={s.contentSection}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          ...
          <p style={s.text}>{STUDENT_VISA.description}</p>

          <div style={s.countryGrid} className="country-grid">
            {STUDENT_VISA.countries.map((country, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} viewport={{ once: true }} style={s.countryCard}
                whileHover={{ borderColor: brand.gold, boxShadow: "0 8px 24px rgba(0,0,0,0.06)" }}>
                <h3 style={s.countryName}><GraduationCap size={18} color={brand.goldDark} />{country.name}</h3>
                <div style={s.featureList}>
                  {country.services.map((service, i) => (
                    <div key={i} style={s.featureItem}><CheckCircle size={14} style={s.featureCheck} /><span>{service}</span></div>
                  ))}
                </div>
              </motion.div>
            ))}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.ctaBox}>
          ...
          <div style={s.ctaContent}>
            <h3 style={s.ctaTitle}>Ready to Start Your Study Abroad Journey?</h3>
            <p style={s.ctaText}>Contact us today for a free consultation. Call {CONTACT_INFO.phone} or email {CONTACT_INFO.email}</p>
