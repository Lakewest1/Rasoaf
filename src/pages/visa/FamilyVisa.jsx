...
import {
  ...
  Loader2, Sparkles, ChevronRight, ArrowDown, Send
} from "lucide-react";
...
const s = {
  ...
  featureGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "16px", marginTop: "24px" },
  ...
  featureTitle: { fontSize: "15px", fontWeight: 600, color: brand.dark, fontFamily: "'Manrope', sans-serif", marginBottom: "6px" },
  featureDesc: { fontSize: "13px", color: brand.mutedText, lineHeight: 1.5 },

  noteBox: { background: brand.goldBg, border: `1px solid ${brand.goldBorder}`, borderRadius: "14px", padding: "20px", marginTop: "24px", display: "flex", gap: "12px", alignItems: "flex-start" },
  noteIcon: { color: brand.goldDark, flexShrink: 0, marginTop: "2px" },
  noteText: { fontSize: "14px", color: brand.mutedText, lineHeight: 1.6 },
...
function FamilyVisaForm() {
  const [formData, setFormData] = useState({
    ...
    destination: "", relationship: "", familyMembers: "", message: "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionData = {
        _subject: `New Family Visa Enquiry - ${formData.firstName} ${formData.lastName}`,
        "First Name": formData.firstName,
        "Last Name": formData.lastName,
        "Email": formData.email,
        "Phone": fullPhone,
        "Destination Country": formData.destination,
        "Relationship to Sponsor": formData.relationship,
        "Number of Family Members": formData.familyMembers || "Not specified",
  ...
  const inp = (name) => ({ ...s.input, ...(focused === name ? s.focusInput : {}), ...(formError && !formData[name] && ["firstName","lastName","email","phone","destination","relationship"].includes(name) ? s.errorInput : {}) });
    ...
    return (
      <div style={s.successWrapper}>
        <div style={s.successImgSide}>
          <img src="https://images.unsplash.com/photo-1511895426328-dc8714191300?w=600&h=900&fit=crop" alt="" style={{ ...s.formImageBg, opacity: 0.35 }} />
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} style={s.successContent}>
          ...
          <h3 style={s.successTitle}>Family Reunification Enquiry Submitted!</h3>
          <p style={s.successMsg}>Thank you for your interest in our family visa services. Our specialist will contact you within <strong>24 hours</strong> to guide you through the process.</p>
          <div style={s.successDetails}>
            ...
            <div style={s.successDetail}><ShieldCheck size={14} color={brand.gold} /><span>Your data is secure & protected</span></div>
          </div>
  ...
  return (
    <div style={s.formWrapper} className="form-flex-container">
      <div style={s.formImageSide} className="form-image-side">
        <img src="https://images.unsplash.com/photo-1511895426328-dc8714191300?w=600&h=900&fit=crop" alt="Happy family" style={s.formImageBg} />
        <div style={s.formImageOverlay} />
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={s.formImageContent}>
          <div style={s.formImageBadge}><Sparkles size={12} color={brand.gold} /><span>FAMILY TOGETHER</span></div>
          <div style={s.formImageIcon}><Heart size={34} color={brand.gold} /></div>
          <h2 style={s.formImageTitle}>Family <span style={s.formImageGold}>Visa</span><br />Request</h2>
          <p style={s.formImageSubtitle}>Reunite with your loved ones. We handle every step of the family visa process with care and expertise.</p>
          <div style={s.formDivider} />
          <div style={s.formImageStats}>
            <div style={s.formImageStat}><div style={s.formImageStatVal}>200+</div><div style={s.formImageStatLbl}>Families Reunited</div></div>
            <div style={s.formImageStat}><div style={s.formImageStatVal}>10+</div><div style={s.formImageStatLbl}>Countries</div></div>
            <div style={s.formImageStat}><div style={s.formImageStatVal}>99%</div><div style={s.formImageStatLbl}>Success Rate</div></div>
          </div>
          <div style={s.formImageFeatures}>
            {[
              { icon: ShieldCheck, text: "Licensed Immigration Advisers" },
              { icon: Users, text: "Full Family Application Support" },
              { icon: Globe, text: "Multi‑country Expertise" },
            ].map((item, i) => {
              const I = item.icon;
      ...
      <div style={s.formSide} className="form-form-side">
        ...
        >
          <div style={s.formHeaderStyle}>
            ...
            <div><div style={s.formHeaderTitle}>Start Your Family Visa</div><div style={s.formHeaderSub}>Fill the form below to get started</div></div>
          </div>
          ...
          <div style={s.formRow}>
            ...
            <div style={s.formGroup}>
              <label style={s.label}>Relationship to Sponsor <span style={s.required}>*</span></label>
              <select name="relationship" value={formData.relationship} onChange={handleChange} required style={s.select}>
                <option value="">Select...</option>
          ...
          <div style={s.formRow}>
            <div style={s.formGroup}>
              <label style={s.label}>Number of Family Members</label>
              <input type="number" min="1" name="familyMembers" value={formData.familyMembers} onChange={handleChange} placeholder="e.g., 3" style={inp("familyMembers")} onFocus={() => setFocused("familyMembers")} onBlur={() => setFocused(null)} />
            </div>
          </div>

          <div style={s.formRow}><div style={s.formGroup}><label style={s.label}>Additional Message</label><textarea name="message" value={formData.message} onChange={handleChange} rows={3} placeholder="Any special requests or questions..." style={s.textarea} onFocus={() => setFocused("message")} onBlur={() => setFocused(null)} /></div></div>
  ...
  return (
    <div style={s.page}>
      ...
      <section style={s.heroSection} className="hero-section">
        <div style={s.heroBg}>
          <img
            src="https://images.unsplash.com/photo-1511895426328-dc8714191300?w=1920&h=800&fit=crop"
        ...
        <div style={s.heroContent}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <span style={s.badge}>
              <Users size={14} color={brand.gold} />
              <span style={s.badgeText}>FAMILY REUNIFICATION</span>
            </span>
            <h1 style={s.heroTitle} className="hero-title">{FAMILY_VISA.heading}</h1>
            <p style={s.heroSubtitle}>{FAMILY_VISA.intro}</p>
      ...
      <section style={s.contentSection}>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          ...
          <p style={s.text}>{FAMILY_VISA.description}</p>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          <div style={s.noteBox}>
            <ShieldCheck size={20} style={s.noteIcon} />
            <p style={s.noteText}>{FAMILY_VISA.note}</p>
          </div>
          <p style={{ ...s.text, marginTop: "20px" }}>{FAMILY_VISA.closing}</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.ctaBox}>
          ...
          <div style={s.ctaContent}>
            <h3 style={s.ctaTitle}>Reunite with Your Family</h3>
            <p style={s.ctaText}>Contact us today for family visa assistance. Call {CONTACT_INFO.phone} or email {CONTACT_INFO.email}</p>
