// src/pages/visa/FamilyVisa.jsx
// ─────────────────────────────────────────────────────────────────────────────
// RASOAF TRAVELS AND TOURS LIMITED — Family Visa Page
...
import {
  Users, Heart, Home, ArrowRight, ShieldCheck, Globe, Clock,
  Loader2, Sparkles, ChevronRight, ArrowDown, Send, ChevronDown
} from "lucide-react";
import { FAMILY_VISA, CONTACT_INFO } from "../../data/visaContent";
...
const relationships = ["Spouse", "Child", "Parent", "Sibling", "Other Relative"];

const familyFeatures = [
  { icon: Heart, title: "Family Reunification", desc: "Bring your loved ones together" },
  { icon: Home, title: "Residence Permits", desc: "Long-term stay solutions" },
  { icon: Users, title: "Dependent Visas", desc: "Spouse and children included" },
  { icon: ShieldCheck, title: "Expert Guidance", desc: "Professional support throughout" },
];
...
const s = {
  ...
  // Features
  featureGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(clamp(140px, 22vw, 220px), 1fr))", gap: "clamp(10px, 1.5vw, 16px)", marginTop: "clamp(16px, 2.5vw, 24px)" },
  ...
  featureTitle: { fontSize: "clamp(13px, 1.2vw, 15px)", fontWeight: 600, color: brand.dark, fontFamily: "'Manrope', sans-serif", marginBottom: "clamp(4px, 0.5vw, 6px)" },
  featureDesc: { fontSize: "clamp(11px, 1vw, 13px)", color: brand.mutedText, lineHeight: 1.5 },
...
//  FAMILY VISA FORM
// ══════════════════════════════════════════════════════════════════════════
function FamilyVisaForm() {
  const [formData, setFormData] = useState({ firstName: "", lastName: "", email: "", phoneCode: "+234", phone: "", destination: "", relationship: "", familyMembers: "", message: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const sd = { _subject: `New Family Visa - ${formData.firstName} ${formData.lastName}`, "First Name": formData.firstName, "Last Name": formData.lastName, "Email": formData.email, "Phone": fp, "Destination": formData.destination, "Relationship": formData.relationship, "Family Members": formData.familyMembers || "N/S", "Message": formData.message || "N/A", "Submitted At": new Date().toLocaleString(), "Page": window.location.href };
  ...
  return (
    <div style={s.page}>
      ...
      <section style={s.heroSection}>
        <div style={s.heroBg}><img src="https://images.unsplash.com/photo-1511895426328-dc8714191300?w=1920&h=800&fit=crop" alt="Family" style={s.heroImg} loading="eager" /></div>
        <div style={s.heroOverlay} />
        <div style={s.heroContent}><motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}><span style={s.badge}><Users size={14} color={brand.gold} /><span style={s.badgeText}>FAMILY REUNIFICATION</span></span><h1 style={s.heroTitle}>{FAMILY_VISA.heading}</h1><p style={s.heroSubtitle}>{FAMILY_VISA.intro}</p><div style={s.heroBtnRow}><button onClick={scrollToForm} style={s.heroBtn} onMouseEnter={e => { e.currentTarget.style.backgroundColor = "#FFE082"; e.currentTarget.style.boxShadow = "0 6px 28px rgba(247,201,72,0.5)"; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = brand.goldLight; e.currentTarget.style.boxShadow = "0 4px 20px rgba(247,201,72,0.35)"; }}><Heart size={20} />Get Started<ArrowDown size={18} /></button></div></motion.div></div>
      </section>
      ...
      <section style={s.contentSection}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          <h2 style={s.sectionTitle}>Keeping Families Together</h2>
          <CollapsibleText text={FAMILY_VISA.description} isMobile={isMobile} />
          <div style={s.featureGrid}>{familyFeatures.map((feat, idx) => { const Icon = feat.icon; return (<motion.div key={idx} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.08 }} viewport={{ once: true }} style={s.featureCard} whileHover={!isMobile ? { borderColor: brand.gold, boxShadow: "0 8px 24px rgba(0,0,0,0.06)" } : {}}><div style={s.featureIcon}><Icon size={22} color={brand.goldDark} /></div><h4 style={s.featureTitle}>{feat.title}</h4><p style={s.featureDesc}>{feat.desc}</p></motion.div>); })}</div>
        </motion.div>

        {/* Note & Closing */}
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={s.card}>
          <div style={s.noteBox}><ShieldCheck size={20} style={s.noteIcon} /><p style={s.noteText}>{FAMILY_VISA.note}</p></div>
          <CollapsibleText text={FAMILY_VISA.closing} isMobile={isMobile} />
        </motion.div>
