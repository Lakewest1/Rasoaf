...
const s = {
  ...
  introBold: { fontWeight: 600, color: brand.dark },
  ...
  packagePriceRow: { marginBottom: "clamp(0.75rem, 1vw, 1rem)" },
  packagePrice: { fontSize: "clamp(1.5rem, 2vw, 1.875rem)", fontWeight: 800, color: brand.dark, fontFamily: "'Manrope', sans-serif", letterSpacing: "-0.02em" },
  packagePerPerson: { fontSize: "clamp(0.7rem, 0.9vw, 0.875rem)", color: brand.gray500, fontFamily: "'Inter', sans-serif" },
  ...
  packageBtn: { display: "block", width: "100%", textAlign: "center", padding: "clamp(0.6rem, 0.9vw, 0.75rem) 0", borderRadius: "12px", color: brand.dark, fontWeight: 600, fontSize: "clamp(0.8rem, 1vw, 0.95rem)", fontFamily: "'Inter', sans-serif", letterSpacing: "0.01em", textDecoration: "none", backgroundColor: brand.goldLight, border: "none", cursor: "pointer", transition: "all 200ms ease", minHeight: "44px" },
...
function CollapsibleText({ text, isMobile }) {
  ...
  if (!shouldTruncate) {
    return <p style={s.introText}>{text}</p>;
  }
    ...
    <div style={s.page}>
      ...
      <section style={s.heroSection}>
        ...
        <div style={s.heroContent}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <span style={s.badge}>
              <Sparkles style={{ width: "clamp(0.85rem, 1vw, 1rem)", height: "clamp(0.85rem, 1vw, 1rem)", display: "inline", marginRight: "0.5rem" }} />
              Blessed Journey
            </span>
            ...
            <motion.button
              ...
              style={s.heroBtn}
              whileHover={{ backgroundColor: "#FFE082", boxShadow: "0 6px 20px rgba(247, 201, 72, 0.45)", transform: "translateY(-2px)" }}
      ...
      <section style={s.introSection}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }} style={s.introCard}>
          ...
          <CollapsibleText 
            text="Whether you wish to perform Umrah monthly, quarterly, or annually, RASOAF Travels and Tours Limited has a special package designed to meet your needs. We proudly offer flexible, affordable, and well-organized Umrah packages for both first-time pilgrims and experienced travellers."
            isMobile={isMobile} 
          />
          
          <CollapsibleText 
            text="Our commitment is to make your journey comfortable, stress-free, and spiritually rewarding. From visa processing and flight reservations to accommodation and travel guidance, we provide comprehensive support throughout your pilgrimage."
            isMobile={isMobile} 
          />
          
          <CollapsibleText 
            text="At RASOAF Travels and Tours Limited, we believe that quality service should be affordable. That is why our loyal clients enjoy competitive pricing and exclusive discounts that make Umrah more accessible without compromising on service quality."
            isMobile={isMobile} 
          />
          
          <CollapsibleText 
            text="We encourage you to compare our services, pricing, and customer support with those of other travel agencies in Nigeria and beyond. We are confident that our professionalism, transparency, personalized attention, and value for money set us apart."
            isMobile={isMobile} 
          />
        ...
        <div style={s.packagesGrid}>
          {umrahPackages.map((pkg, idx) => (
            ...
            >
              ...
              <button onClick={() => scrollToSection("booking-form-section")} style={{ ...s.packageBtn, backgroundColor: pkg.color, color: brand.white }}
                onMouseEnter={!isMobile ? (e) => { e.currentTarget.style.opacity = "0.9"; } : undefined}
          ...
          <motion.div initial={{ opacity: 0, x: isMobile ? 0 : 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={s.formCard}>
            ...
            <form action="https://formspree.io/f/YOUR_FORM_ID" method="POST" style={{ display: "flex", flexDirection: "column", gap: "clamp(0.75rem, 1.2vw, 1.25rem)" }}>
              ...
              <div style={s.formGroup}>
                <label htmlFor="first-name" style={s.label}>First Name <span style={s.required}>*</span></label>
                <div style={s.inputWrapper}>
                  <User style={s.inputIcon} />
                  <input id="first-name" type="text" name="firstName" placeholder="Saheed" required style={s.input} onFocus={handleFocus} onBlur={handleBlur} />
                </div>
              </div>

              {/* Last Name */}
              <div style={s.formGroup}>
                <label htmlFor="last-name" style={s.label}>Last Name <span style={s.required}>*</span></label>
                <div style={s.inputWrapper}>
                  <User style={s.inputIcon} />
                  <input id="last-name" type="text" name="lastName" placeholder="Mohammed" required style={s.input} onFocus={handleFocus} onBlur={handleBlur} />
                </div>
              </div>

              {/* Email */}
              <div style={s.formGroup}>
                <label htmlFor="email" style={s.label}>Email Address <span style={s.required}>*</span></label>
                <div style={s.inputWrapper}>
                  <Mail style={s.inputIcon} />
                  <input id="email" type="email" name="email" placeholder="Mohammed@example.com" required style={s.input} onFocus={handleFocus} onBlur={handleBlur} />
                </div>
              </div>
              ...
              <div style={s.formGroup}>
                ...
                <div style={s.phoneRow}>
                  <div style={s.phoneCodeWrap}>
                    <select name="countryCode" defaultValue="+234" style={{ ...s.select, paddingLeft: "0.75rem" }} onFocus={handleFocus} onBlur={handleBlur}>
                      {countryCodes.map((code) => (<option key={code.value} value={code.value}>{code.label}</option>))}
                    </select>
                    <ChevronDown style={{ ...s.selectIcon, right: "0.5rem" }} />
                  </div>
                  <input id="phone" type="tel" name="phone" placeholder="800 123 4567" style={s.phoneInput} onFocus={handleFocus} onBlur={handleBlur} />
                </div>
              </div>
              ...
              <div style={s.dateRow}>
                <div style={s.formGroup}>
                  <label htmlFor="departure" style={s.label}>Preferred Departure <span style={s.required}>*</span></label>
                  <input id="departure" type="date" name="preferredDeparture" required style={{ ...s.input, paddingLeft: "0.75rem" }} onFocus={handleFocus} onBlur={handleBlur} />
                </div>
                <div style={s.formGroup}>
                  <label htmlFor="return-date" style={s.label}>Return Date</label>
                  <input id="return-date" type="date" name="returnDate" style={{ ...s.input, paddingLeft: "0.75rem" }} onFocus={handleFocus} onBlur={handleBlur} />
                </div>
              </div>

              {/* Preferred Contact */}
              <div style={s.formGroup}>
                <label htmlFor="preferred-contact" style={s.label}>Preferred Contact <span style={s.required}>*</span></label>
                <div style={s.inputWrapper}>
                  <select id="preferred-contact" name="preferredContact" required defaultValue="email" style={s.select} onFocus={handleFocus} onBlur={handleBlur}>
                    {contactMethods.map((method) => (<option key={method.value} value={method.value}>{method.label}</option>))}
                  </select>
                  <ChevronDown style={s.selectIcon} />
                </div>
              </div>
              ...
              <div style={s.tripleRow}>
                <div style={s.formGroup}>
                  <label htmlFor="adults" style={s.label}>Adults <span style={s.required}>*</span></label>
                  <div style={s.inputWrapper}>
                    <Users style={s.inputIcon} />
                    <input id="adults" type="number" name="adults" min="1" defaultValue="1" required style={s.input} onFocus={handleFocus} onBlur={handleBlur} />
                  </div>
                </div>
                <div style={s.formGroup}>
                  <label htmlFor="children" style={s.label}>Children (2-11)</label>
                  <div style={s.inputWrapper}>
                    <Baby style={s.inputIcon} />
                    <input id="children" type="number" name="children" min="0" defaultValue="0" style={s.input} onFocus={handleFocus} onBlur={handleBlur} />
                  </div>
                </div>
                <div style={s.formGroup}>
                  <label htmlFor="infants" style={s.label}>Infants (0-2)</label>
                  <div style={s.inputWrapper}>
                    <Baby style={s.inputIcon} />
                    <input id="infants" type="number" name="infants" min="0" defaultValue="0" style={s.input} onFocus={handleFocus} onBlur={handleBlur} />
                  </div>
                </div>
              </div>
              ...
              {/* Umrah Package */}
              <div style={s.formGroup}>
                <label htmlFor="umrah-package" style={s.label}>Umrah Package <span style={s.required}>*</span></label>
                <div style={s.inputWrapper}>
                  <select id="umrah-package" name="umrahPackage" required defaultValue="" style={s.select} onFocus={handleFocus} onBlur={handleBlur}>
                    {umrahPackageOptions.map((opt) => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                  </select>
                  <ChevronDown style={s.selectIcon} />
                </div>
              </div>
              ...
              {/* Preferred Month */}
              <div style={s.formGroup}>
                <label htmlFor="preferred-month" style={s.label}>Preferred Month</label>
                <div style={s.inputWrapper}>
                  <select id="preferred-month" name="preferredMonth" defaultValue="" style={s.select} onFocus={handleFocus} onBlur={handleBlur}>
                    {preferredMonthOptions.map((opt) => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                  </select>
                  <ChevronDown style={s.selectIcon} />
                </div>
              </div>
              ...
              {/* Message */}
              <div style={s.formGroup}>
                <label htmlFor="message" style={s.label}>Additional Message</label>
                <textarea id="message" name="additionalMessage" placeholder="Any special requests or requirements..." rows={3} style={s.textarea} onFocus={handleFocus} onBlur={handleBlur} />
              </div>

              {/* Submit */}
              <motion.button type="submit" style={s.submitBtn}
                whileHover={{ backgroundColor: "#FFE082", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}
                whileTap={{ backgroundColor: brand.goldLight }}
              >
